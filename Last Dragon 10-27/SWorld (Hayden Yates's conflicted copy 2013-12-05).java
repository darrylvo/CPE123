...
public class SWorld extends World
{
    ...
    public void showDeath(String s, int x, int y) {
        Visual visual = new Visual(s);
        addObject(visual,x,y);
        int scoreX = (scoreboard.getX() + scoreboard.getWidth())/2 - visual.getX();
        int scoreY = visual.getY() - (scoreboard.getY() + scoreboard.getHeight())/2;
        visual.setLocation(visual.getX()+scoreX/10,visual.getY()+scoreY/10);
        if(visual.getY() <(scoreboard.getY()+scoreboard.getHeight)) {
            removeObject(visual);
        }
    }
